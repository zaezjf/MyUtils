...
Version    : python 3.9
Description: Luckframe数据库连接操作脚本
    使用方法(命令行)：python db_base_V1.py [数据库连接配置信息] [操作方法] [sql语句] [mongoDb集合名称]
    示例：
    - mysql
    ```
    # 用户ysapp_boss连接MySQL，查询ysapp库注册用户表"sf_app_register"手机号184开头的最新一条信息
    python db_base_V1.py "db_type=mysql&db_client=10.213.40.118:3306&db_name=ysapp&db_userName=ysapp_boss&db_userPwd=12Fj2uix)iu" "select" "SELECT * FROM sf_app_register WHERE MOBILE  LIKE '184%' LIMIT 1"
    ```
    - oracle
    ```
    ...
    python db_base_V1.py "db_type=oracle&db_client=10.213.32.74:1521&db_name=yspos_portal&db_userName=yspos_portal&db_userPwd=123456&db_service=orcl" "select" "SELECT * FROM (SELECT * FROM PORTAL.MOBILE_DYNAMIC_CODE_NEW WHERE USERCODE LIKE '184%' ORDER BY EXPIRETIME DESC) WHERE ROWNUM < 2"
    ```
    - mongoDb
    ```
    # 用户bottomAdmin连接mongoDb，查询bottom库
    python db_base_V1.py "db_type=mongo&db_client=10.213.40.58:27017&db_name=bottom&db_userName=bottomAdmin&db_userPwd=bottom1234567" "select" "find({\"mercId\":\"826440345119153\"}).limit(1)" "mh_app_h5_use_stats"
    ```
    参数说明：
    ...
    def select_db(self, sql, mongo_collection: str = None):
        ...
        if db_type == "mongo":
            ...
            if not mongo_collection is None:
                ...
                print("mongoDb执行sql查询需要指定查询表(集合)")
...
if __name__ == "__main__":
    ...
    #
    # db_sql_oracle = "SELECT * FROM (SELECT * FROM PORTAL.MOBILE_DYNAMIC_CODE_NEW WHERE USERCODE LIKE '184%' ORDER BY EXPIRETIME DESC) WHERE ROWNUM < 2"
    # db_config_mysql = "db_type=mysql&db_client=10.213.40.118:3306&db_name=ysapp&db_userName=ysapp_boss&db_userPwd=12Fj2uix)iu"
    # db_sql_mysql = "SELECT * FROM sf_app_register WHERE MOBILE  LIKE '184%' LIMIT 1"
    ...
    # db_sql_mongo = 'find({"mercId":"826440345119153"}).limit(1)'
